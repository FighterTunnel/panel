...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 pt-1">
        ...
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item text-dark">Edit Servers</li>
        <!--end::Item-->
    </ul>
    <!--end::Breadcrumb-->
@endsection
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="d-flex flex-column-fluid">
            ...
            <div id="kt_content_container" class="container-xxl">
                <form id="edit_server_form" class="form d-flex flex-column flex-lg-row"
                    data-kt-redirect="{{ route('admin.servers.index') }}"
                    action="{{ route('admin.servers.update', $server->id) }}" method="PUT">
                                        ...
                                        @if ($category->children->count() > 0)
                                            @foreach ($category->children as $child)
                                                <option value="{{ $child->id }}"
                                                    {{ $child->id == $server->category_id ? 'selected' : '' }}>
                                                    {{ $child->name }}</option>
                                            @endforeach
                                        @elseif ($category->parent == null)
                                            <option value="{{ $category->id }}"
                                                {{ $category->id == $server->category_id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group row">
                                            ...
                                            <select name="country_id" class="form-select" data-control="select2"
                                                ...
                                                @foreach ($countries as $country)
                                                    <option value="{{ $country->id }}"
                                                        {{ $server->country_id == $country->id ? 'selected' : '' }}>
                                                        {{ $country->name }}</option>
                                                @endforeach
                                            </select>
                                            <!--end::Input-->
                                        ...
                                        <div class="mb-5 fv-row fv-plugins-icon-container col-md-6">
                                            ...
                                            <select name="type" class="form-select" data-control="select2"
                                                ...
                                                <option value="">Select an option</option>
                                                <option value="3" {{ $server->type == '3' ? 'selected' : '' }}>3 Days
                                                <option value="5" {{ $server->type == '5' ? 'selected' : '' }}>5 Days
                                                <option value="7" {{ $server->type == '7' ? 'selected' : '' }}>7 Days
                                                <option value="30" {{ $server->type == '30' ? 'selected' : '' }}>30
                                                    Days
                                            </select>
                                            <!--end::Input-->
                                            <!--begin::Description-->
...
@section('scripts')
    ...
    <script>
        const repeater = $('#ports_repeater').repeater({
            initEmpty: false,
        ...
        repeater.setList([
            @foreach (json_decode($server->ports, true) as $key => $port)
                {
                    'ports_name': '{{ $key }}',
                    'ports_number': '{{ $port }}',
                },
